...
import com.kineo.web.dto.MedicalInsuranceCompanyDto;
import com.kineo.web.dto.MedicalInsurancePlanDto;
import com.kineo.web.exception.EntityNotFoundException;
import com.kineo.web.exception.ValidationException;
import com.kineo.web.request.medical_insurance.MedicalInsuranceCompanyRequest;
import com.kineo.web.request.medical_insurance.MedicalInsurancePlanRequest;
...
public class MedicalInsuranceController {
    ...
    public PaginationResponse<MedicalInsuranceCompanyDto> getCompanies(@RequestParam(value = "limit", required = false) Integer limit,
        ...
        response.setItems(companies.stream().map(company -> new MedicalInsuranceCompanyDto(company)).collect(Collectors.toList()));
    ...
    public PaginationResponse<MedicalInsurancePlanDto> getPlans(@RequestParam(value = "limit", required = false) Integer limit,
        ...
        response.setItems(plans.stream().map(plan -> new MedicalInsurancePlanDto(plan)).collect(Collectors.toList()));
        Paging paging = new Paging();
        paging.setLimit(maxResult);
        paging.setOffset(fistResult);
