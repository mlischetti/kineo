...
import com.kineo.process.ProcessFactory;
...
public class CalendarEventSyncJob extends BaseJob {
    ...
    @Value("${kineo.calendar.calendar-event-sync-job.enabled}")
        ...
        if (enabled) {
            ...
            if (CollectionUtils.isNotEmpty(appointments)) {
                appointments.forEach(appointment -> {
                    com.kineo.process.Process process = processFactory.create(appointment);
                    if (process != null) {
